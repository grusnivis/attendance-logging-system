...
import MergeCopiesTeacher
import MergeCopiesStudent
...
def start():
    global signal, rfid, name, event, serial
    ...
    serial = getSerial()
    # removes RFID storage csv file when user goes back to the start/home page
    if os.path.exists(serial + '_toVerify.csv'):
        os.remove(serial + '_toVerify.csv')

    MergeCopiesTeacher.merge_teacher()
    sleep(1)
    update_list()
...
def regi():
    ...
    ex = Button(root, text="EXIT", padx=20, pady=5, height=3, width=16, command=root.destroy)
    ex.config(font="Arial, 15", state=DISABLED)
    ex.place(x=70, y=360)
...
def teacher_enrollment():
    ...
    def choice(_):
        ...
        match = selected.split(", ")[0] + "," + selected.split(", ")[1]
        display_name = selected.split(", ")[1]
    ...
    while scanning_start:
        ...
        if id_scanned == 1:
            ...
            if not reg_rfid == "None":
                ...
                dup = t_master_copy.duplicate(reg_rfid, s_copy, t_copy)
                if dup == 1:
                    with open(t_copy, 'r') as f:
                        lines = f.readlines()
                    ...
                    clear_frame()
                    root['background'] = 'red'
                    lbl = Label(root, text=f"RFID ALREADY\nEXISTS IN\nDATABASE", font="Arial 65  bold",
                                background="red",
                                foreground="white")
                    lbl.place(relx=0.5, rely=0.5, anchor="center")
    ...
    while scanning_start:
        ...
        if id_scanned == 1:
            ...
            if not rfid_tag2 == "None":
                # checks for duplicates
                dup = t_master_copy.duplicate(rfid_tag2, s_copy, t_copy)
                if dup == 1:
                    with open(t_copy, 'r') as f:
                        lines = f.readlines()
                        for i, line in enumerate(lines):
...
def school_event():
    ...
    global newfile, logs, class_file
    newfile = (year + "-" + month + "-" + day + "_" + name + "_ACTIVITY_SCHOOL EVENT_OUT OF CLASS_" +
               hour + "-" + mins + "-" + sec + ".csv")
    logs = "./" + newfile
    class_file = ""
...
def school_event2():
    ...
    menu.menu.add_command(label='SHUT DOWN', font='Arial, 25', command=shutdown)
    Button(root, text="BACK", font="Arial, 18", highlightthickness=0,
           borderwidth=0, command=back_check).place(anchor="nw")
...
def event_scanning():
    ...
    while signal:
        ...
        if id_scanned == 1:
            ...
            #validates if student RFID is found in masterlist
            if s_master_copy.validate(event_rfid):
                ...
                try:
                    check_in_log = s_master_copy.append(event_rfid, id2, name2, newfile, hour, mins, sec, "PRESENT")
                    ...
                    if check_in_log == 1:
                        ...
                        b.config(text=f"{name2}\nYOU ALREADY SCANNED", foreground="red")
                        root.update()
                        sleep(1)
                ...
                try:
                    if not event_rfid == "None":
                        check_in_log = s_master_copy.append(event_rfid, "", ",", newfile, hour, mins, sec, "PRESENT")
...
def teacher_scan():
    ...
    """for file in os.listdir('/home/pi/ALS_SHARED/'):
        if file.endswith("_toVerify.csv"):
            # get modification time of the file
            other_teacher_login = os.path.getmtime(file)

            # if file was modified more than 7 mins ago
            if time.time() - other_teacher_login > 420:
                os.remove(file)
        else:
            root.after(300, teacher_scanning)"""
    ...
    while scanning_start:
        ...
        if id_scanned == 1:
            ...
            if t_master_copy.validate(rfid):
                ...
                for file in os.listdir('/home/pi/ALS_SHARED/'):
                    if file.endswith("_toVerify.csv") and not file.startswith(serial):
                        with open(file) as f:
                            getRFID = f.readline()
                            rfid_check.append(getRFID)
                        f.close()
                ...
                if rfid_check:
                    for check in rfid_check:
                        if check == rfid:
                            clear_frame()
                            root['background'] = 'red'
                            lbl = Label(root, text=f"ACCESS DENIED\nYOU ARE ALREADY\nUSING ANOTHER\nDEVICE",
                                        font="Arial 55  bold",
                                        background="red", foreground="white")
                            lbl.place(relx=0.5, rely=0.5, anchor="center")
                            root.after(3000, start)
                            root.after(3000, lambda: root.config(background="#d9d9d9"))

                        else:
                            root.after(300, teacher_scanning2)
                else:
                    root.after(300, teacher_scanning2)
...
def teacher_scanning2():
    # stores rfid in a txt .csv file
    store_rfid = serial + '_toVerify.csv'
    with open(store_rfid, 'w') as fp:
        fp.write(rfid)
    fp.close()
...
def select_class():
    ...
    menu.menu = Menu(menu, tearoff=0)
    menu["menu"] = menu.menu
    menu.menu.add_command(label='REFRESH', font='Arial, 25', command=select_class)
    menu.menu.add_command(label='RETURN', font='Arial, 25', command=start)
    #menu.menu.add_command(label='EXIT', font='Arial, 25', command=root.destroy)
    menu.menu.add_command(label='RESTART', font='Arial, 25', command=restart)
    menu.menu.add_command(label='SHUT DOWN', font='Arial, 25', command=shutdown)
...
def display_selection():
    ...
    menu.menu = Menu(menu, tearoff=0)
    menu["menu"] = menu.menu
    menu.menu.add_command(label='REFRESH', font='Arial, 25', command=display_selection)
    menu.menu.add_command(label='RETURN', font='Arial, 25', command=start)
    ...
    display2 = Label(root, text=f"GROUP:\t\t{course}\nCOURSE CODE:\t{group}\nSCHEDULE:\t{schedule}",
...
def back_check():
    ...
    title = Label(root, text="CANCEL ATTENDANCE TAKING?", font="Arial, 33")
    title.pack(padx=35, pady=(60, 25))

    ver = LabelFrame(root, text="Please verify", font="Arial, 15", padx=15)
    ver.pack()

    prompt = Label(ver, text="ATTENDANCE IS ONGOING.\n GOING BACK WILL DELETE\n THE EXISTING ATTENDANCE LOG",
                   font="Arial, 20")
    prompt.pack(padx=65, pady=25)

    yes = Button(root, text="YES", padx=20, pady=5, height=3, width=16, command=buffer3)
    yes.config(font="Arial, 15")
    yes.place(x=490, y=360)

    if event == 1:
        no = Button(root, text="NO", padx=20, pady=5, height=3, width=15, command=school_event2)
    else:
        no = Button(root, text="NO", padx=20, pady=5, height=3, width=15, command=student_scan)

    no.config(font="Arial, 15")
    no.place(x=70, y=360)


#this deletes the created attendance log
def buffer3():
    if os.path.exists(f'/home/pi/{newfile}'):
        os.remove(f'/home/pi/{newfile}')
    start()
...
def student_scanning():
    ...
    while signal:
        ...
        if id_scanned == 1:
            ...
            #validate if student RFID is found in the masterlist
            if s_master_copy.validate(rfid2):
                ...
                try:
                    # check if the student ID is found in the class list
                    valid = s_list.get_user(id2)
                    if valid == 1:
                        # checks if the RFID of student in attendance logs is present
                        check_in_log = s_master_copy.append(rfid2, id2, name2, newfile, hour, mins, sec, student_status)
                ...
                try:
                    if not rfid2 == "None":
                        check_in_log = s_master_copy.append(rfid2, "", ",", newfile, hour, mins, sec, student_status)
                        b.config(text=f"MATCH NOT FOUND", foreground="red")
                        root.update()
                        sleep(1)
                except FileNotFoundError:
...
def buffer2():
    if os.path.exists(newfile):
        os.remove(newfile)
        MergeCopiesStudent.merge_student()
        display_selection()
        os.chdir('/home/pi/ALS_SHARED/')
    else:
        MergeCopiesStudent.merge_student()
...
def student_enrollment():
    ...
    try:
        ...
        def choice(_):
            ...
            if tag == "" or tag == "\n":
                inst.config(text=f"{display_name}:\nRFID NOT ENROLLED", foreground="red", font="Arial, 23")
    ...
    while scanning_start:
        ...
        if id_scanned == 1:
            ...
            if not reg_rfid == "None":
                ...
                #checks for duplicates
                dup = s_master_copy.duplicate(reg_rfid,s_copy,t_copy)
                if dup == 1:
                    with open(s_copy, 'r') as f1:
                        lines = f1.readlines()
                        for i, line in enumerate(lines):
                    ...
                    check_in_log = s_master_copy.append(reg_rfid, id_key.split(',')[0], logging, newfile, hour, mins, sec, student_status)
                    ...
                    clear_frame()
                    root['background'] = 'red'
                    lbl = Label(root, text=f"RFID ALREADY\nEXISTS IN\nDATABASE", font="Arial 65  bold", background="red",
                                foreground="white")
                    lbl.place(relx=0.5, rely=0.5, anchor="center")
    ...
    while scanning_start:
        ...
        if id_scanned == 1:
            ...
            if not rfid_tag2 == "None":
                scanning_start = 0
                # checks for duplicates
                dup = s_master_copy.duplicate(rfid_tag2, s_copy, t_copy)
                if dup == 1:
                    ...
                    if trigger == 0:
                        ...
                        logging = copy.strip().split(',')[2] + "," + copy.strip().split(',')[3]
                        check_in_log = s_master_copy.append(rfid_tag2, id, logging, newfile, hour, mins, sec, student_status)
...
def end_attendance():
    ...
    try:
        ...
        for file in os.listdir('/home/pi/ALS_SHARED/'):
            if file.endswith("_toVerify.csv") and file.startswith(serial):
                os.remove(file)
...
def end():
    config()
    converted_start = datetime.datetime.strptime(year + "-" + month + "-" + day + "_" + class_start, "%Y-%m-%d_%H:%M\n")
    start = converted_start - datetime.timedelta(minutes=10)
    stop = datetime.datetime.strptime(year + "-" + month + "-" + day + "_" + class_end, "%Y-%m-%d_%H:%M\n")
    time_now = datetime.datetime.now()

    #if time_now == stop or time_now > stop:
    if time_now >= stop or time_now <= start:
            return 0
    else:
        return 1
...
def update_list():
    global s_masterFile, t_masterFile, t_copy, s_copy, t_master_copy, s_master_copy
